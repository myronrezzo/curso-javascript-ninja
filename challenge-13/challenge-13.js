(function() {
	...
	*/
	...
	var newSul = brasil.filter( function( bItem ) {
		return sul.some( function( sItem ) {
			return bItem === sItem;
		});
	});
	...
	*/
	var newSudeste = [];
	brasil.forEach( function(bItem, index, arr) {
		if ( sudeste.some( function(sItem) { return bItem === sItem }) ) {
			newSudeste.push( bItem );
			brasil = brasil.filter( function(item) {
				return (bItem === item) ? false : true;
			} );
		}
	});
	...
	*/
	...
	var todosEstados7Letras = brasil.every( function(item) {
		return item.toString().length > 7;
	});
	
	if (todosEstados7Letras) {
	...
	*/
	...
	var cearaExiste = brasil.some( function(item) {
		return item === 'Ceará';
	});
